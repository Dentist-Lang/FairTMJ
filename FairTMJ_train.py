"""FairTMJ with prediction alignment, adversarial alignment and warm-up.
...
from fairness_aa import AA_WEIGHT, GRL_BETA, Adversary, adversarial_alignment_loss
from fairness_pa import PA_WEIGHT, prediction_alignment_loss


EPOCHS = 80
FOLDS = 5
# Reference parameters. The first candidate wins an exact CV-score tie.
REFERENCE_CONFIG = dict(lr=1e-4, batch_size=8, pa_weight=PA_WEIGHT,
                       aa_weight=AA_WEIGHT, beta=GRL_BETA, warmup_epochs=10)


DEFAULT_SEEDS = (0, 1, 2, 3, 4)
DEFAULT_CV_SEED = 8
# One-parameter-at-a-time search around the reference configuration.
# The learning-rate candidates match those used by the baseline.
DEFAULT_CANDIDATES = (
    {}, {"lr": 5e-5}, {"lr": 2e-4},
    {"pa_weight": 0.1}, {"pa_weight": 0.4},
    {"aa_weight": 0.05}, {"aa_weight": 0.2},
)
DEFAULT_OUTPUT_DIR = "outputs/FairTMJ"
...
def move_batch(batch, device):
    closed, opened, labels, sex, patients, sides = batch
    return ([x.to(device) for x in closed], [x.to(device) for x in opened],
            labels.to(device), sex.to(device), patients, sides)


def warmup_scale(epoch_number, warmup_epochs):
    return min(1.0, epoch_number / warmup_epochs) if warmup_epochs else 1.0


def fit(dataset, config, seed, device, workers, log_path):
    """Train only; deliberately has no validation/test loader or checkpoint ranking."""
    setup_seed(seed)
    model = SingleTaskModel().to(device)
    adversary = Adversary(input_dim=model.feature_dim).to(device)
    optimizer = torch.optim.Adam(list(model.parameters()) + list(adversary.parameters()),
                                 lr=config["lr"])
    ...
    with log_path.open("x", newline="") as writer:
        log = csv.DictWriter(writer, fieldnames=["epoch", "alpha", "loss", "cls_loss",
                                                "pa_loss", "aa_loss", "train_acc"])
        log.writeheader()
        for epoch in range(1, EPOCHS + 1):
            model.train()
            adversary.train()
            alpha = warmup_scale(epoch, config["warmup_epochs"])
            totals = np.zeros(4)
            correct = count = 0
            for batch in loader:
                closed, opened, labels, sex, _, _ = move_batch(batch, device)
                optimizer.zero_grad()
                logits, hidden = model(closed, opened)
                cls = nn.functional.cross_entropy(logits, labels)
                pa = prediction_alignment_loss(logits, sex)
                aa = adversarial_alignment_loss(hidden, sex, adversary, beta=config["beta"])
                loss = cls + alpha * (config["pa_weight"] * pa + config["aa_weight"] * aa)
                if not torch.isfinite(loss):
                    raise RuntimeError(f"Non-finite training loss at epoch {epoch}")
                loss.backward()
                optimizer.step()
                n = labels.numel()
                totals += np.array([loss.item(), cls.item(), pa.item(), aa.item()]) * n
                correct += (logits.argmax(dim=1) == labels).sum().item()
                count += n
            row = dict(zip(["loss", "cls_loss", "pa_loss", "aa_loss"], (totals / count).tolist()))
            row.update(epoch=epoch, alpha=alpha, train_acc=correct / count)
            log.writerow(row)
            writer.flush()
...
def evaluate(model, dataset, batch_size, device, workers):
    ...
    with torch.no_grad():
        for batch in make_loader(dataset, batch_size, None, workers, shuffle=False):
            closed, opened, labels, sex, patients, sides = move_batch(batch, device)
            logits, _ = model(closed, opened)
            probabilities = logits.softmax(dim=1).cpu().tolist()
            predictions = logits.argmax(dim=1).cpu().tolist()
            labels_list, sex_list = labels.cpu().tolist(), sex.cpu().tolist()
            labels_all.extend(labels_list)
            probs_all.extend(probabilities)
            for p, pred, label, s, patient, side in zip(probabilities, predictions, labels_list,
                                                      sex_list, patients, sides):
                rows.append(dict(prob_class_0=p[0], prob_class_1=p[1], prob_class_2=p[2],
                                 y_true=label, joint_id=patient, side=side,
                                 y_pred_label=pred, sex_code=s))
...
def load_candidates(path=None):
    ...
    for entry in overrides:
        ...
        for key, value in config.items():
            ...
            if value < 0 or (key in ("lr", "batch_size") and value == 0):
                raise ValueError(f"Invalid parameter {key}: {value}")
        if type(config["batch_size"]) is not int or type(config["warmup_epochs"]) is not int:
            raise ValueError("batch_size and warmup_epochs must be integers")
...
def cross_validate(dataset, args, device):
    ...
    protocol = dict(method="FairTMJ", epochs=EPOCHS, n_folds=FOLDS, selection_metric="mean_fold_accuracy",
    ...
    for index, config in enumerate(candidates):
        ...
        for fold, (train_idx, valid_idx) in enumerate(folds):
            ...
            model, adversary = fit(Subset(dataset, train_idx), config, args.cv_seed,
...
def final_runs(dataset, selected, args, device):
    if selected["protocol"].get("method") != "FairTMJ":
        raise ValueError("Use a FairTMJ CV selection file")
    ...
    for seed in args.seeds:
        ...
        model, adversary = fit(dataset, config, seed, device, args.workers, run_dir / "train_log.csv")
        torch.save(model.state_dict(), run_dir / "FairTMJ_epoch80.pth")
        torch.save(adversary.state_dict(), run_dir / "adversary_epoch80.pth")
    ...
    if args.test_file is None:
        return
    test_records = load_records(args.test_file, include_sex=True)
    ...
    test_dataset = ADDMRIDataset(args.test_data_folder or args.data_folder,
                                 records=test_records, include_sex=True)
    run_metrics = []
    for seed in args.seeds:
        run_dir = final_dir / f"seed_{seed}"
        model = SingleTaskModel(pretrained=False).to(device)
        model.load_state_dict(torch.load(run_dir / "FairTMJ_epoch80.pth", map_location=device, weights_only=True))
...
def parse_args(argv=None):
    ...
    parser.add_argument("--candidates", help="Optional JSON list replacing the 7 built-in CV candidates")
...
def main(argv=None):
    ...
    dataset = ADDMRIDataset(args.data_folder, file=args.train_file, include_sex=True)
    if args.mode == "final":
